...
class RobustGitHubCLI:
            ...
            if result.returncode == 0:
                ...
                if "github.com" in url:
                    if url.startswith("https://github.com/"):
                        repo_part = url.replace("https://github.com/", "").replace(
                            ".git", ""
                        )
                    elif url.startswith("git@github.com:"):
                        repo_part = url.replace("git@github.com:", "").replace(
                            ".git", ""
                        )
                    else:
                        return None
    ...
    def run_gh_command(
        self, args: list[str], fallback_to_api: bool = True
    ...
    def get_workflow_status(self) -> dict[str, Any]:
        ...
        runs_data = self.run_gh_command(
            [
                ...
                "10",
                "--json",
                "status,conclusion,headBranch,workflowName,createdAt,url",
            ]
        )
    ...
    def get_pr_status(self, pr_number: str) -> dict[str, Any]:
        ...
        pr_data = self.run_gh_command(
            [
                ...
                pr_number,
                "--json",
                "title,state,mergeable,statusCheckRollup,number,url",
            ]
        )
        ...
        check_summary = {
            ...
            "pending": [
                c for c in checks if c.get("status") in ["QUEUED", "IN_PROGRESS"]
            ],
