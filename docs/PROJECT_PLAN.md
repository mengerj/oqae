...
**Last Updated**: 2026-06-29 — PR #9 slice 1: benchmarking harness added under
`src/omvqvae/benchmark/` (pure metrics — codebook utilization, separability,
reconstruction NLL/MAE — plus a `run_suite` harness over the training loop and a
Markdown comparison-table report), with example 4 and offline tests at 100%.
**Current Focus**: PR #9 — benchmarking & scaling. Slice 1 (metrics/reporting
scaffold) done; next slices wire real likelihood/codebook sweeps and Census
streaming throughput, then write the benchmark report. See `docs/STATUS.md`.
